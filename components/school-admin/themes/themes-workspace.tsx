...
export function ThemesWorkspace() {
  ...
  const years = useQuery<PrimaryAcademicYear[]>({ queryKey: ["school-admin", "academic-years"], queryFn: backendApi.adminPrimaryAcademicYears });
  const yearId = years.data?.find((year) => year.is_active)?.id ?? years.data?.[0]?.id ?? "";
  const themes = useQuery<PrimaryCurriculumTheme[]>({ queryKey: ["school-admin", "themes", level], queryFn: () => backendApi.adminPrimaryThemes(level) });
  ...
  async function runConfirmed() {
    ...
    try {
      if (confirm.kind === "customize") {
        const copy = await backendApi.adminCustomizePrimaryTheme(confirm.theme.id);
        await refresh();
        ...
        await backendApi.adminArchivePrimaryTheme(confirm.theme.id);
        await refresh();
        updateUrl({ theme: null });
        toast({ title: "Theme archived" });
      } else {
        await backendApi.adminDeletePrimaryTheme(confirm.theme.id);
        await refresh();
        updateUrl({ theme: null });
      ...
      {themes.isError || lessons.isError || years.isError ? <PageError description="Themes and their curriculum usage could not be loaded." onRetry={() => { void themes.refetch(); void lessons.refetch(); void years.refetch(); }} /> : themes.isLoading ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map((item) => <Skeleton key={item} className="h-64 rounded-3xl" />)}</div> : visible.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((theme) => <ThemeCard key={theme.id} theme={theme} count={themeLessons(theme, lessons.data ?? []).length} onOpen={() => updateUrl({ theme: theme.id })} onConfirm={setConfirm} onDuplicate={async () => {
            try { const copy = await backendApi.adminDuplicatePrimaryTheme(theme.id); await refresh(); updateUrl({ theme: copy.id }); } catch (error: any) { toast({ title: "Could not duplicate theme", description: error?.message, variant: "error" }); }
          }} />)}
        </div>
...
function ThemeCard({ theme, count, onOpen, onConfirm, onDuplicate }: { theme: PrimaryCurriculumTheme; count: number; onOpen: () => void; onConfirm: (action: ConfirmAction) => void; onDuplicate: () => void }) {
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative h-28 bg-gradient-to-br from-blue-50 via-violet-50 to-amber-50 p-5"><span className="text-4xl" aria-hidden="true">{theme.emoji || "🎨"}</span><span className={cn("absolute right-4 top-4 rounded-full px-2.5 py-1 text-[11px] font-bold", ownership === "teachpad" ? "bg-white text-blue-700" : ownership === "customized" ? "bg-violet-100 text-violet-700" : "bg-emerald-100 text-emerald-700")}>{ownershipLabel(ownership)}</span></div>
      <div className="p-5">
...
function ThemeDetail({ theme, lessons, yearId, level, onBack, onRefresh, onConfirm, confirm, busy, runConfirmed }: { theme: PrimaryCurriculumTheme; lessons: PrimaryCurriculumLesson[]; yearId: string; level: string; onBack: () => void; onRefresh: () => Promise<void>; onConfirm: (action: ConfirmAction) => void; confirm: ConfirmAction; busy: boolean; runConfirmed: () => Promise<void> }) {
  ...
  async function addTopic() {
    ...
    setSaving(true);
    try { await backendApi.adminCreatePrimaryTopic(theme.id, { name: newTopic.trim(), subtheme: null, description: null, position: topics.length, keywords: [], aliases: [], is_active: true }); setNewTopic(""); await onRefresh(); }
  ...
  async function moveTopic(index: number, direction: -1 | 1) {
    ...
    try { await Promise.all([backendApi.adminUpdatePrimaryTopic(topics[index].id, { position: target }), backendApi.adminUpdatePrimaryTopic(topics[target].id, { position: index })]); await onRefresh(); }
    ...
    <SchoolAdminPage>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
          ...
          <div className="mt-5 space-y-2">{topics.length ? topics.map((topic, index) => <TopicRow key={topic.id} topic={topic} index={index} count={topics.length} editable={schoolOwned} onMove={moveTopic} onRefresh={onRefresh} />) : <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-10 text-center text-sm text-slate-500">No topics yet.</div>}</div>
