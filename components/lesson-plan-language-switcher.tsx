...
type LessonPlanLanguageSwitcherProps = {
  /** Languages that already exist, primary first. */
  availableLanguages: string[];
  activeLanguage: LessonPlanLanguage;
  /** Language currently being generated, if any. */
  translatingLanguage?: LessonPlanLanguage | null;
  /** True when the active variant's source has been edited since translating. */
  isActiveStale?: boolean;
  onSelect: (language: LessonPlanLanguage) => void;
  onTranslate: (language: LessonPlanLanguage) => void;
};

/**
 * Language tabs for one lesson plan: existing languages switch instantly (a read),
 * missing ones show a "+" that spends a generation to translate.
 *
 * Labels sit in each language's own script — a teacher looking for the Urdu
 * version scans for اردو, not the word "Urdu".
 */
export function LessonPlanLanguageSwitcher({
  availableLanguages,
  activeLanguage,
  translatingLanguage,
  isActiveStale,
  onSelect,
  onTranslate,
}: LessonPlanLanguageSwitcherProps) {
  const available = new Set(availableLanguages);
  // Chrome follows the language being read, so the control is legible to a
  // teacher who does not read English.
  const locale = getLessonPlanLocale(activeLanguage);
