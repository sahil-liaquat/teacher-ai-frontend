...
import { lessonIssues, lessonsForMonth, monthLabel, SCHOOL_LEVELS, SCHOOL_MONTHS } from "@/lib/school-admin-curriculum";
...
export function MasterOverview() {
  ...
  const issues = visible.flatMap((lesson) => lessonIssues(lesson).map((issue) => ({ lesson, issue })));
  const mapped = visible.reduce((count, lesson) => count + (lesson.steps ?? []).filter((step) => (step.resource_ids?.length ?? 0) > 0).length, 0);
  const blocks = visible.reduce((count, lesson) => count + (lesson.steps?.length ?? 0), 0);
  const readiness = visible.length ? Math.round((visible.filter((lesson) => lessonIssues(lesson).length === 0).length / 25) * 100) : 0;
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <MetricCard label="Teaching days" value={visible.length} icon={<BookOpen className="h-4 w-4" />} />
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 space-y-2">{issues.slice(0, 8).map(({ lesson, issue }, index) => <Link key={`${lesson.id}-${issue}-${index}`} href={`${ROOT}/design?year=${yearId}&level=${level}&month=${month}&day=${lesson.id}`} className="flex items-center gap-3 rounded-xl border border-amber-100 bg-amber-50 p-3"><AlertTriangle className="h-4 w-4 shrink-0 text-amber-600" /><span className="min-w-0 flex-1"><b className="block truncate text-sm text-slate-900">{lesson.title || "Untitled teaching day"}</b><span className="text-xs text-amber-800">{issue}</span></span><ArrowRight className="h-4 w-4" /></Link>)}{!issues.length && <p className="rounded-xl bg-emerald-50 p-4 text-sm font-bold text-emerald-700">No readiness blockers in this month.</p>}</div>
      </section>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-black text-slate-950">Recently edited</h2><div className="mt-4 grid gap-3 md:grid-cols-3">{[...(lessons.data ?? [])].sort((a, b) => b.version - a.version || b.id.localeCompare(a.id)).slice(0, 6).map((lesson: PrimaryCurriculumLesson) => <Link key={lesson.id} href={`${ROOT}/design?year=${yearId}&level=${level}&month=${lesson.month}&day=${lesson.id}`} className="rounded-xl border border-slate-200 p-3"><span className="text-xs font-bold uppercase text-slate-400">{lesson.status}</span><b className="mt-1 block truncate text-sm">{lesson.title || lesson.daily_focus || "Untitled teaching day"}</b></Link>)}</div></section>
  </div>;
}
