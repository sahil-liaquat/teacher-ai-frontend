...
export function DashboardMyClasses() {
  ...
  const recentChapters = home.data?.recent_chapters || [];
    ...
    <section aria-labelledby="dashboard-my-classes-title" className="mx-auto w-full max-w-[1240px] px-4">
      <div className="rounded-[20px] border border-white/70 bg-white/80 p-4 shadow-[0_14px_34px_rgba(15,23,42,0.06)] backdrop-blur-sm sm:p-5">
        ...
        {home.isLoading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {recentChapters.slice(0, 3).map((item) => {
              ...
              return (
                <Link key={`${item.workspace_id}:${item.topic.id}`} href={href} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_24px_rgba(30,50,80,0.04)] transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_15px_34px_rgba(37,99,235,0.09)]">
