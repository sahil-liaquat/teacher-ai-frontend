...
const statCards = [
  ...
  { label: "Worksheets Created", fallback: "0", sub: "Total", icon: FileText, tone: "green" },
    ...
    <div className="mx-auto grid w-full max-w-[1480px] gap-4 px-0 2xl:px-4">
      <header className="mx-auto flex w-full max-w-[1240px] flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className={cn(
          ...
          <h1 className={cn(
            "flex flex-wrap items-center gap-x-2 text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl",
          ...
          statCards.map((stat, index) => (
            index === 1 && worksheets.isError ? (
              ...
              <StatCard
                key={stat.label}
                {...stat}
                sub={index === 3 ? `Used in ${new Date().toLocaleString("default", { month: "long" })}` : stat.sub}
                value={
                  index === 0
                ...
                <div className="flex-1 flex items-end justify-between gap-2 px-1 min-h-[96px]">
                  {Array.from({ length: 7 }).map((_, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center justify-end gap-2 h-full">
                      <div className="w-full max-w-[40px] rounded-t-lg bg-slate-200" style={{ height: `${25 + Math.random() * 50}%` }} />
