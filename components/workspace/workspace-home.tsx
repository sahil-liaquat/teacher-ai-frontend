  ...
  return (
    <main className="workspace-reference-page mx-auto w-full max-w-[1240px] space-y-8 px-3 py-4 sm:px-6 lg:px-7">
      ...
      <section id="recent-chapters" aria-labelledby="recent-chapters-title">
        <div className="flex items-center justify-between"><h2 id="recent-chapters-title" className="text-lg font-black text-fg">Recent Chapters</h2><span className="text-xs font-semibold text-slate-400">Latest 5 generations</span></div>
        <div className="mt-4 flex gap-3 overflow-x-auto border-b border-slate-200 pb-0">{(["all", "in_progress", "not_started", "completed"] as ChapterFilter[]).map((value) => <button key={value} type="button" onClick={() => setFilter(value)} className={cn("border-b-2 px-4 pb-3 text-micro font-bold transition", filter === value ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800")}>{value === "all" ? "All" : value === "in_progress" ? "In Progress" : value === "not_started" ? "Not Started" : "Completed"}</button>)}</div>
      ...
      <section aria-labelledby="recent-resources-title" className="rounded-[20px] border border-slate-200 bg-white p-5 shadow-[0_8px_28px_rgba(30,50,80,0.04)]">
        <div className="flex items-center justify-between"><h2 id="recent-resources-title" className="text-base font-black text-fg">Recently Used Resources</h2><Link href="/dashboard/resources" className="text-xs font-bold text-blue-600">View all</Link></div>
        {recentResources.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{recentResources.map(({ item, resource }) => <RecentResourceCard key={`${item.workspace_id}:${resource.type}:${resource.generation_id}`} item={item} resource={resource} />)}</div> : <p className="mt-5 rounded-card bg-slate-50 px-4 py-7 text-center text-xs font-semibold text-slate-400">Created resources will appear here.</p>}
      </section>
    </main>
  );
}
...
function RecentResourceCard({ item, resource }: { item: WorkspaceHomeTopic; resource: WorkspaceResource }) {
  ...
  return <Link href={href} className="flex items-center gap-3 rounded-card border border-slate-200 p-3 transition hover:border-blue-200 hover:bg-blue-50/30"><span className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-card", tool.tone)}><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block truncate text-micro font-black text-fg">{item.chapter_title}</span><span className="mt-0.5 block truncate text-micro font-semibold text-slate-600">{resourceLabels[resource.type]}</span><span className="mt-1.5 block text-micro font-medium text-slate-400">{relativeTime(resource.generated_at || item.last_opened_at)}</span></span><MoreVertical className="h-4 w-4 shrink-0 text-slate-400" /></Link>;
}
