  ...
  return (
    <main className="workspace-reference-page mx-auto w-full max-w-[1260px] space-y-4 px-3 pb-24 pt-2 sm:px-6 sm:pb-7 sm:pt-3">
      <Link
        ...
        className="inline-flex h-8 w-fit items-center gap-1.5 rounded-control border border-slate-200 bg-white px-3 text-micro font-black text-fg shadow-sm transition hover:border-blue-200 hover:text-blue-700"
      >
      ...
      <section className="grid overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_12px_36px_rgba(30,50,80,0.06)] md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center px-6 py-6 sm:px-8 sm:py-8">
          <h1 className="text-3xl font-black tracking-[-0.045em] text-fg sm:text-h1">{workspace.chapter_title}</h1>
          ...
          <div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-emerald-50 px-3 py-1 text-micro font-bold text-emerald-700">✦ Curiosity</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-micro font-bold text-emerald-700">✦ Competency based</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-micro font-bold text-emerald-700">✦ NEP Aligned</span></div>
          <div className="mt-5 flex flex-wrap gap-4 text-micro font-semibold text-slate-500"><span className="flex items-center gap-2"><Clock3 className="h-4 w-4" />Last opened {relativeTime(workspace.last_opened_at)}</span><span className="text-slate-300">|</span><span>⚑ Target: {topic.scheduled_at ? teachingDate(topic.scheduled_at) : "This week"}</span></div>
        </div>
      </section>

      {workspace.is_archived ? <div className="flex items-center justify-between gap-3 rounded-card border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-bold text-amber-900"><span>This workspace is archived. Everything remains available.</span><button type="button" onClick={() => state.workspaceMutation.mutate({ is_archived: false })} className="underline">Restore workspace</button></div> : null}
      ...
      <section id="chapter-resources" aria-labelledby="resources-title" className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-[0_10px_32px_rgba(30,50,80,0.04)] sm:p-5">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-3"><h2 id="resources-title" className="text-lg font-black text-fg">Chapter Resources</h2><span className="text-micro font-semibold text-slate-500">ⓘ Complete all resources for a ready-to-teach chapter</span></div><div className="flex w-fit rounded-card bg-slate-100 p-1"><button type="button" onClick={() => setResourceView("cards")} className={cn("inline-flex h-8 items-center gap-2 rounded-control px-3 text-micro font-black", resourceView === "cards" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500")}><Grid2X2 className="h-3.5 w-3.5" />Cards</button><button type="button" onClick={() => setResourceView("timeline")} className={cn("inline-flex h-8 items-center gap-2 rounded-control px-3 text-micro font-black", resourceView === "timeline" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500")}><List className="h-3.5 w-3.5" />Timeline</button></div></div>
      ...
      <section aria-labelledby="timeline-title"><h2 id="timeline-title" className="text-lg font-black text-fg">Chapter Preparation Timeline</h2><div className="mt-5 overflow-x-auto pb-2"><ol className="flex min-w-[760px]">{resources.map((resource, index) => <TimelineStep key={resource.type} resource={resource} index={index} total={resources.length} />)}</ol></div></section>

      <section aria-labelledby="recent-generations-title"><div className="flex items-center justify-between"><h2 id="recent-generations-title" className="text-sm font-black text-fg">Recent Generations for this Chapter</h2><Link href="/dashboard/recent-generations" className="text-micro font-bold text-blue-600">View all →</Link></div>{readyResources.length ? <div className="mt-3 grid gap-3 md:grid-cols-3">{readyResources.slice(0, 3).map((resource) => <RecentGeneration key={resource.type} workspace={workspace} topic={topic} resource={resource} />)}</div> : <p className="mt-3 rounded-card border border-dashed border-slate-200 px-4 py-7 text-center text-xs font-semibold text-slate-400">Generated chapter resources will appear here.</p>}</section>

      <section className="flex flex-wrap items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 px-5 py-4"><span className="mr-3 text-xs font-black text-fg">More Actions</span><button type="button" onClick={() => window.print()} className="inline-flex h-9 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-micro font-black text-slate-700"><Download className="h-3.5 w-3.5" />Export All</button><Link href="/dashboard/textbooks" className="inline-flex h-9 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-micro font-black text-slate-700"><span className="text-base text-blue-600">+</span>Create New Chapter</Link></section>
...
function CompactResourceRow({ resource, index, workspace, topic }: { resource: WorkspaceResource; index: number; workspace: TeachingWorkspace; topic: WorkspaceTopic }) {
  ...
  return <Link href={href} className="flex items-center gap-4 rounded-card border border-slate-200 px-4 py-3 hover:border-blue-200 hover:bg-blue-50/30"><span className="grid h-7 w-7 place-items-center rounded-full bg-slate-100 text-micro font-black text-slate-500">{index + 1}</span><span className={cn("grid h-9 w-9 place-items-center rounded-card", visual.tone)}><Icon className="h-4 w-4" /></span><span className="flex-1 text-xs font-black text-slate-800">{resourceLabels[resource.type]}</span><span className={cn("text-micro font-bold", resource.status === "ready" ? "text-emerald-600" : resource.status === "generating" ? "text-blue-600" : "text-slate-400")}>{resource.status === "ready" ? "Created" : resource.status === "generating" ? "Generating" : "Pending"}</span><ArrowRight className="h-4 w-4 text-slate-400" /></Link>;
}
...
function RecentGeneration({ workspace, topic, resource }: { workspace: TeachingWorkspace; topic: WorkspaceTopic; resource: WorkspaceResource }) {
  ...
  return <Link href={href} className="flex items-center gap-3 rounded-card border border-slate-200 bg-white px-4 py-3 transition hover:border-blue-200"><span className={cn("grid h-10 w-10 place-items-center rounded-card", visual.tone)}><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-micro font-black text-slate-800">{workspace.chapter_title}</span><span className="mt-0.5 block text-micro font-bold text-slate-600">{resourceLabels[resource.type]}</span><span className="mt-1 block text-micro text-slate-400">{resource.generated_at ? relativeTime(resource.generated_at) : "Created"}</span></span><ArrowRight className="h-4 w-4 text-slate-400" /></Link>;
}
