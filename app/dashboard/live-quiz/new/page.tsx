...
export default function CreateLiveQuizPage() {
  ...
  if (loading) {
    return (
      <div className="mx-auto grid min-h-[70vh] max-w-[720px] place-items-center">
        <div className="w-full rounded-[24px] border border-[#ffe1d2] bg-white p-8 text-center shadow-[0_18px_48px_var(--teachpad-shadowCard)]">
          <LoaderCircle className="mx-auto h-12 w-12 animate-spin text-[#e57820]" />
  ...
  return (
    <div className="mx-auto max-w-[1120px]">
      <div className="overflow-hidden rounded-[18px] border border-[#ffe1d2] bg-white shadow-[0_14px_34px_var(--teachpad-shadowCard)]">
        <header className="relative min-h-[180px] border-b border-[#ffe1d2] bg-gradient-to-br from-[#fffaf0] via-white to-[#ffe1d2] px-5 py-6 sm:px-6">
          <div className="relative z-10 max-w-[620px]">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#ffe1d2] bg-white/80 px-3 py-1.5 text-xs font-black text-[#e57820] shadow-sm">
        ...
        <div className="grid gap-4 p-4 sm:p-5">
            ...
            <div className="mt-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {questionTypes.map((type) => (
                  <button key={type} type="button" onClick={() => toggleType(type)} className={cn("min-h-12 rounded-xl border px-3 text-left text-sm font-black transition", selectedTypes.includes(type) ? "border-[#ff8a3d] bg-[#fffaf0] text-[#e57820] ring-4 ring-[#ffe1d2]/60" : "border-teachpad-cardBorder bg-white text-teachpad-muted hover:border-[#ffe1d2]")}>
                    {type}
                  </button>
                ))}
              </div>
          ...
          <div className="rounded-xl border border-[#ffe1d2] bg-[#fffaf0] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button type="button" onClick={handleGenerate} className="bg-gradient-to-r from-[#ff8a3d] to-[#eb3b5a] sm:min-w-[190px]">
  ...
  return (
    <section className="rounded-[16px] border border-teachpad-cardBorder bg-white p-4 shadow-[0_10px_24px_rgba(30,80,90,0.04)]">
      <div className="mb-4 flex items-start gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#fff0bf] text-[#e57820]"><Icon className="h-5 w-5" /></span>
        <div>
...
function Toggle({ label, active, locked, onClick }: { label: string; active: boolean; locked?: boolean; onClick: () => void }) {
  return (
    <button type="button" disabled={locked} onClick={onClick} className={cn("flex min-h-12 items-center justify-between rounded-xl border px-3 text-left text-sm font-black transition disabled:cursor-not-allowed", active ? "border-[#c7f7ed] bg-[#ecfff7] text-[#0b7f53]" : "border-teachpad-cardBorder bg-white text-teachpad-muted hover:border-[#ffe1d2]")}>
      {label}
      <span className={cn("h-6 w-11 rounded-full p-1 transition", active ? "bg-[#1fbc79]" : "bg-slate-200")}>
