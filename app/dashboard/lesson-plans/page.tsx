...
export default function LessonPlansListPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1240px] space-y-8 px-4 py-4">
      ...
      {plans.isLoading ? <Card><CardContent className="p-6 text-sm font-semibold text-fg-muted">Loading saved lesson plans...</CardContent></Card> : null}
      ...
      <div className="grid gap-4">
        {plans.data?.items?.length ? plans.data.items.map((plan, index) => (
          <Link key={plan.id} href={`/dashboard/lesson-plans/${plan.id}`} className="clickable-card premium-hover-sm reveal-card relative overflow-hidden rounded-[18px] border border-white/70 bg-gradient-to-br from-blue-50 via-blue-50 to-white p-4 shadow-[0_14px_34px_rgba(15,23,42,0.07)] [--clickable-card-hover-bg:linear-gradient(135deg,#dbeafe_0%,#ffffff_74%)]" style={{ animationDelay: `${index * 60}ms` }}>
            <div className="absolute -left-8 -top-8 h-24 w-24 rounded-full bg-blue-200/30 blur-2xl" />
            <div className="flex flex-col gap-3.5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-start gap-3 min-w-0 flex-1">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-card bg-white/88 text-primary shadow-[0_14px_30px_rgba(139,92,246,0.18),inset_0_1px_0_rgba(255,255,255,0.92)] ring-1 ring-teachpad-lavender sm:h-12 sm:w-12 sm:rounded-2xl">
                  <BookOpen className="h-5.5 w-5.5 sm:h-6 sm:w-6" />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-slate-955 text-sm sm:text-sm leading-snug break-words">
                    {plan.topic}
                  </h3>
                  <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                    <Badge className="text-micro px-1.5 py-0.5 font-semibold">Lesson Plan</Badge>
                    <Badge className="bg-emerald-50 text-green-800 text-micro px-1.5 py-0.5 font-semibold">{plan.subject}</Badge>
                  </div>
        ...
        )) : !plans.isLoading ? <Card><CardContent className="p-8 text-center"><div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-violet-50 text-primary"><BookOpen className="h-7 w-7" /></div><h3 className="text-lg font-black text-slate-950">No lesson plans yet.</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-600">Generate your first textbook-grounded lesson plan.</p><Link href="/dashboard/lesson-plans/new"><Button className="mt-5">Create Lesson Plan</Button></Link></CardContent></Card> : null}
      </div>
    </div>
  );
}
