...
import { backendApi, getRateLimitNotice, isFreeQuotaError, isPaymentRequiredError, type PresentationGeneratePayload } from "@/lib/api";
import {
  fetchChaptersCached,
  useBoards,
  useBooksByClass,
...
const themeOptions = [
  { id: "Light", label: "Light", desc: "A light touch", bg: "bg-white border-teachpad-red text-fg", descColor: "text-fg-muted/85", titleColor: "text-pink-600 font-black" },
  ...
  { id: "Maths", label: "Maths", desc: "Numbers are fun", bg: "bg-amber-50 border-amber-100 text-slate-800", descColor: "text-[#5c5440]/85", titleColor: "text-sky-600 font-black" },
  { id: "Simple", label: "Simple", desc: "Less is more", bg: "bg-[#fcfcfd] border-zinc-200 text-slate-900", descColor: "text-slate-400", titleColor: "text-amber-800 font-black" },
...
export default function PresentationGeneratorPage() {
  ...
  useEffect(() => {
    ...
    toast({
      ...
      description: getErrorMessage(boardsQuery.error, "Please try again."),
      variant: "error"
    });
        ...
        <div className="relative min-h-[100px] overflow-hidden rounded-t-[18px] border-b border-teachpad-red bg-gradient-to-br from-rose-50 via-white to-rose-100 px-4 py-4 sm:min-h-[130px] sm:px-6 sm:py-5">
          {step === 1 ? (
            <div className="relative z-10 max-w-[560px]">
              ...
              <p className="mt-2.5 max-w-[520px] text-sm font-medium leading-6 text-fg-muted">Generate textbook-grounded classroom presentations with visuals, speaker notes, and activities.</p>
            </div>
          ) : step === 2 ? (
            <div className="relative z-10 max-w-[560px]">
              ...
              <p className="mt-2.5 max-w-[520px] text-sm font-medium leading-6 text-fg-muted">Fine-tune your presentation by choosing the style, tone, detail level, and sections to include.</p>
            </div>
          ) : (
            <div className="relative z-10 max-w-[560px]">
              <div className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-teachpad-red bg-white px-3 py-1.5 text-xs font-semibold text-fg-muted shadow-sm">
              ...
              <p className="mt-2.5 max-w-[520px] text-sm font-medium leading-6 text-fg-muted">Select a premium theme for your presentation slides. Emojis, background gradients, and elements will adapt dynamically.</p>
            </div>
          )}
        ...
        <div className="p-4 sm:p-5">
          ...
          {step === 3 && (
            <div key="step-3" className="animate-slide-in-right space-y-6">
                  ...
                  {themeOptions.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.id === "Classic" && (
                          <>
                            <div className="absolute top-[-10%] right-[-10%] w-16 h-16 rounded-full bg-red-300/18 blur-md" />
                            <div className="absolute bottom-[-10%] left-[-10%] w-14 h-14 rounded-full bg-slate-300/25 blur-md" />
              ...
              {/* Success Card */}
              <div className="rounded-2xl border border-teachpad-red bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm mb-6">
...
function FieldCard({ icon: Icon, label, required, color = "blue", children }: { icon: React.ComponentType<{ className?: string }>; label: string; required?: boolean; color?: string; children: ReactNode }) {
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-teachpad-red bg-white p-4 shadow-sm sm:p-5">
