...
const faqs = [
  {
    ...
    answer:
      "It means TeachPad generates content using the actual textbook, chapter, and curriculum you select.",
    Icon: HelpCircle,
  },
  {
...
function HeroSection() {
  ...
  return (
    <section className="grid min-h-[430px] overflow-hidden rounded-[32px] border border-blue-100 bg-[radial-gradient(circle_at_16%_12%,#f7fbff_0,transparent_32%),radial-gradient(circle_at_86%_20%,#dfeeff_0,transparent_34%),linear-gradient(135deg,#ffffff_0%,#f4faff_100%)] shadow-[0_24px_70px_rgba(30,82,153,0.12)] lg:grid-cols-[0.86fr_1.14fr]">
      <motion.div
        initial={prefersReduced ? "visible" : "hidden"}
        animate="visible"
        variants={staggerContainer}
        className="relative z-10 px-8 py-14 sm:px-12 sm:py-16 lg:px-16 lg:py-20"
      >
        <motion.h1
          variants={staggerItem}
          className="max-w-xl text-[44px] font-black leading-none text-slate-950 sm:text-[58px] lg:text-[68px]"
        >
          Boards &
          ...
          >
            <div className="hero-float">
              <Image
                ...
                height={941}
                priority
                className="h-auto w-full max-w-[620px] object-contain drop-shadow-[0_34px_54px_rgba(47,79,129,0.18)] lg:absolute lg:right-[-46px] lg:top-1/2 lg:max-w-[810px] lg:-translate-y-1/2"
              />
            </div>
          </motion.div>
        </ParallaxTilt>
...
function StatsSection() {
  return (
    <StaggerGroup className="mt-16 sm:mt-24 lg:mt-32 grid rounded-[32px] border border-slate-200 bg-white px-8 py-10 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:grid-cols-2 lg:grid-cols-4">
      {stats.map((stat, index) => (
        <StaggerFadeItem key={stat.label}>
          <div className={`flex items-center gap-5 py-2 ${index > 0 ? "lg:border-l lg:border-slate-200 lg:pl-10" : ""}`}>
...
function BoardsSection() {
  return (
    <section className="mt-16 sm:mt-24 lg:mt-32 rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:p-12 lg:p-16">
      <RevealOnScroll>
      ...
      <StaggerGroup className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {boards.map((board) => (
          <StaggerScaleItem key={board.title} className="flex flex-col">
            <Card3DTilt maxTilt={5} className="flex-1 flex flex-col rounded-2xl">
...
function BoardCard({ board }: { board: (typeof boards)[number] }) {
  ...
  return (
    <article className="group flex flex-1 min-h-[360px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-none transition-all duration-300">
...
function GroundingSection() {
  ...
  return (
    <section className="mt-16 sm:mt-24 lg:mt-32 rounded-[32px] border border-blue-100 bg-[linear-gradient(135deg,#ffffff_0%,#eff7ff_100%)] p-8 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:p-12 lg:p-16">
      <RevealOnScroll>
        <div className="grid items-center gap-6 lg:grid-cols-[220px_1fr_380px]">
          ...
          >
            {groundingSteps.map((step) => (
              <motion.li key={step} variants={checklistItem} className="flex items-center gap-3 text-sm font-bold text-[#07133b]">
...
function CtaBand() {
  return (
    <section className="mt-16 sm:mt-24 lg:mt-32 rounded-[32px] border border-slate-200 bg-[linear-gradient(135deg,#ffffff_0%,#eef7ff_100%)] p-8 shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:p-12 lg:p-16">
      <RevealOnScroll>
        <div className="grid items-center gap-6 lg:grid-cols-[1fr_420px]">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              ...
              <Link href="/#features" className="inline-flex h-12 items-center justify-center rounded-full border border-blue-200 bg-white px-6 text-sm font-black text-blue-600 shadow-[0_14px_30px_rgba(15,23,42,0.06)] hover:-translate-y-0.5 hover:border-blue-300"
                style={{
...
function FaqSection() {
  return (
    <section className="py-16 sm:py-24 lg:py-32">
      ...
      <StaggerGroup className="mt-5 grid gap-3 lg:grid-cols-2">
        {faqs.map((faq) => (
          <StaggerFadeItem key={faq.question}>
            <article className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_10px_24px_rgba(25,78,140,0.05)]">
